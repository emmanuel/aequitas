...
describe Aequitas::VirtusIntegration::ClassMethods do
  let(:class_under_test) do
    Class.new do
      ...
      attribute :validated_attribute, String, :format => :email_address
  ...
  describe '#valid?' do
    ...
    describe 'when a string that looks like an email address' do
      let(:attribute_value) { 'address@domain.tld' }
      it('is valid') { assert_predicate subject, :valid? }
    end

    describe "when a string that doesn't look like an email address" do
