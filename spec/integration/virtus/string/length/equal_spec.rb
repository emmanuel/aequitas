...
describe Aequitas::VirtusIntegration::ClassMethods do
  let(:class_under_test) do
    Class.new do
      ...
      attribute :validated_attribute, String, :length => 3
  ...
  describe '#valid?' do
    ...
    describe 'when a string of length equal to the expected' do
      let(:attribute_value) { 'foo' }
      it('is valid') { assert_predicate subject, :valid? }
    end

    describe 'when a string of length not equal to the expected' do
